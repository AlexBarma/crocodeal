<!--C:\OSPanel\domains\crocodeal.org\resources\views\search\inc\posts\template\grid.blade.php-->
@php
	$posts ??= [];
	$totalPosts ??= 0;
@endphp
@if (!empty($posts) && $totalPosts > 0)
	@foreach($posts as $key => $post)
		<div class="item-list">
			...
			<div class="row"><!--Сделана голубая рамка для списка объявлений-->
				<div style="border: 3px solid #4682B4;
				border-radius: 12px;"  
				class="col-sm-2 col-12 no-padding photobox">
					<div class="add-image">
						...
						<div class="col-12 m-0 p-0 d-flex justify-content-end">
							@if (!empty(data_get($post, 'latestPayment.package')))
								@if (data_get($post, 'latestPayment.package.has_badge') == 1)
									<a class="btn btn-success btn-sm make-favorite"><!--поменял цвет выделения обЪявления btn-success-->
										<i class="fa fa-certificate"></i> <span>{{ data_get($post, 'latestPayment.package.short_name') }}ХИТ ПРОДАЖ</span>
									</a>&nbsp;
								@endif
							@endif
