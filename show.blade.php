<!-- Путь C:\OSPanel\domains\crocodeal.org\resources\views\account\messenger\show.blade.php -->
{{--
                                ...
                                <div class="user-bar-top">
                                    <div class="user-top">
                                        <p>
                                            ...
                                            <a href="{{ \App\Helpers\UrlGen::post(data_get($thread, 'post')) }}">
                                    ...
                                    <div class="message-tool-bar-right float-end call-xhr-action">
                                        <div class="btn-group btn-group-sm">
                                            ...
                                            <a href="{{ url('account/messages/' . data_get($thread, 'id') . '/actions?type=markAsNotImportant') }}" class="btn btn-secondary markAsNotImportant" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ t('Mark as not important') }}">
                                                <i class="fas fa-star"></i>
                                            </a>
                                            @else
                                            <a href="{{ url('account/messages/' . data_get($thread, 'id') . '/actions?type=markAsImportant') }}" class="btn btn-secondary markAsImportant" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ t('Mark as important') }}">
                                                <i class="far fa-star"></i>
                                            </a>
                                            @endif
                                            <a href="{{ url('account/messages/' . data_get($thread, 'id') . '/delete') }}" class="btn btn-secondary" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ t('Delete') }}">
                                                <i class="fas fa-trash"></i>
                                            </a>
                                            @if (data_get($thread, 'p_is_unread'))
                                            <a href="{{ url('account/messages/' . data_get($thread, 'id') . '/actions?type=markAsRead') }}" class="btn btn-secondary markAsRead" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ t('Mark as read') }}">
                                                <i class="fas fa-envelope"></i>
                                            </a>
                                            @else
                                            <a href="{{ url('account/messages/' . data_get($thread, 'id') . '/actions?type=markAsUnread') }}" class="btn btn-secondary markAsRead" data-bs-toggle="tooltip" data-bs-placement="top" title="{{ t('Mark as unread') }}">
                                    ...
                                    <div class="type-message">
                                        <div class="type-form">
                                            ...
                                            <form id="chatForm" role="form" method="POST" action="{{ $updateUrl }}" enctype="multipart/form-data">
                                                ...
                                                <textarea id="body" name="body" maxlength="500" rows="3" class="input-write form-control" placeholder="{{ t('Type a message') }}" style="{{ (config('lang.direction')=='rtl') ? 'padding-left' : 'padding-right' }}: 75px;"></textarea>
                                                <div class="button-wrap">
                                                    <!-- <input id="addFile" name="filename" type="file"> Убрал кнопку отправки картинок в пользовательском чате -->
...
<script>
    var loadingImage = '{{ url('
    images / loading.gif ') }}';
    var loadingErrorMessage = '{{ t('
    Threads could not be loaded ') }}';
    var actionErrorMessage = '{{ t('
    This action could not be done ') }}';
    var title = {
        'seen': '{{ t('
        Mark as read ') }}',
        'notSeen': '{{ t('
...
<script>
    ...
    $('#addFile').fileinput({
        ...
        allowedFileExtensions: {
            !!getUploadFileTypes('file', true) !!
        },
        maxFileSize: {
            {
                (int) config('settings.upload.max_file_size', 1000)
            }
        },
